...
- For microstrip, CPW, stripline, and grounded coplanar feeds, the waveguide `Port` must be placed on the line's transverse feed cross-section, not merely on the outer calculation boundary for convenience and not merely on the metal trace end face. For a standard grounded microstrip, size the port width to a documented multiple of the feed-line width, and make the main port height extend from the signal conductor to the reference ground. The port span or picked aperture must include the signal conductor, dielectric region, and all relevant reference grounds; add air/field extension only when the CST help, macro pattern, or specific feed physics requires it. Add a mode line from signal conductor to reference ground. If this cannot be created or verified, stop with `port_setup_error` and do not present the project as complete.
